...
describe("BFG deterministic Catalog to Batch assignment", () => {
  ...
  it("backfills 100 existing customer items and assigns a future preorder once", async () => {
    ...
    expect(await admin.query(api.batchTracking.listUnassignedForAdmin, { batchId: batch.batchId })).toEqual([]);
  ...
  it("keeps zero, ambiguous, and locked receiving targets unassigned", async () => {
    ...
    await expect(
      admin.query(api.batchTracking.listUnassignedForAdmin, { batchId: firstBatch.batchId }),
    ).resolves.toEqual(expect.arrayContaining([expect.objectContaining({ assignmentState: "Tujuan Batch ambigu" })]));
    ...
    await expect(
      admin.query(api.batchTracking.listUnassignedForAdmin, { batchId: lockedBatch.batchId }),
    ).resolves.toEqual(expect.arrayContaining([expect.objectContaining({ assignmentState: "Belum masuk Batch" })]));
  });
