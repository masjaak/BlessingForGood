...
describe("Clerk identity and BFG authorization", () => {
  ...
  it("enforces operational RBAC and protects owner-only boundaries", async () => {
    const t = testConvex();
    const owner = t.withIdentity(ownerIdentity);
    const customer = t.withIdentity(customerIdentity);
    ...
    await owner.mutation(api.users.ensureCurrentUser, {});
    const secondCustomerUser = await secondCustomer.mutation(api.users.ensureCurrentUser, {});
    ...
    await expect(
      owner.mutation(api.users.updateRole, { userId: secondCustomerUser.appUserId, role: "customer" }),
    ...
    await expect(owner.mutation(api.users.updateRole, { userId: ownerUser.appUserId, role: "admin" })).rejects.toThrow(
      "OWNER_PROTECTED",
    );
  ...
  it("fails closed for Admin data and writes across every identity state", async () => {
    ...
    await expect(
      owner.mutation(api.users.updateRole, { userId: customerUser.appUserId, role: "admin" }),
    ).resolves.toMatchObject({ role: "admin" });
    await expect(
