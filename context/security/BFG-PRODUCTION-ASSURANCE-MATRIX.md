...
| Security headers                   | `REMEDIATED_GREEN` pending deployment | CSP, HSTS existing, nosniff, Referrer-Policy, Permissions-Policy, XFO/COOP added in `next.config.ts` | Safe Production header recheck required after deployment; CSP uses documented `unsafe-inline` compatibility exception |
...
| Public HTTP scale                  | `VALIDATED_TO_CAPACITY_500`           | Profile A passes 10/50/100/300/500; 750 stopped at 6.90% errors                                      | 750 bottleneck; 1,000 not run                                                                                         |
...
Before code correction deployment: `REMEDIATED_GREEN pending deployment`.
After tests, push, Convex/Vercel Production deployment, and safe header/auth
smoke, update the row to `GREEN_EVIDENCE` with the final commit/deployment
anchor. Do not leave a confirmed security fix only in the working tree.
