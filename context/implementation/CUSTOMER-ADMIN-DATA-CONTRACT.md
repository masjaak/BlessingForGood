...
| Secret Catalog | `secretCatalogs`, `catalogItems` | `catalogAccess.unlock/getUnlocked/listAccessible` returns only an active member grant and safe catalog view | `secretCatalogs.list` and catalog item queries | `secretCatalogs.create/open/close/createBundle`, `catalogItems.add` | Open/closed catalog and granted contents update reactively | Customer must have `catalog.read`; private product data stays server-side until grant; admin/owner manages catalog | READY_SHARED_BACKEND |
| Catalog access code | `catalogAccessCodes` (`codeDigest`, `lookupDigest`, status, expiry) | No customer query exposes code records or hashes | No raw code read; operational state is server-side | `catalogAccess.generateCode`, `catalogAccess.revokeCode`, legacy-compatible `setCode` | Revoked/expired codes cannot create new grants; current grants follow existing expiry/closure semantics | Admin/owner `catalog.manage`; BFG pepper is Convex environment-only | READY_SHARED_BACKEND |
| Catalog grant | `catalogAccessGrants` (`appUserId`, `catalogId`, `grantedAt`, `expiresAt`, `revokedAt`) | `catalogAccess.getUnlocked/listAccessible` checks member identity, active grant, and open catalog before returning products | `secretCatalogs.list` plus grant records are server-authorized | `catalogAccess.unlock` creates/refreshes a grant; no customer grant mutation | Authorized customer catalog query updates when grant/catalog state changes | Grant is tied to active `appUserId`; browser storage is only a navigation hint | READY_SHARED_BACKEND |
