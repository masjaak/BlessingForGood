...
5. `appUsers` provisioning resolves to active, suspended, or admission-required;
   membership reconciliation separately distinguishes auth loading, approved
   invitation pending, no application, and review pending.
...
state. An approved request that is still reconciling renders activation-pending
guidance rather than a new Join CTA.
