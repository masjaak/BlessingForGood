...
| Catalog access code | active, revoked, expired/invalid | generate, redeem, revoke, expiry; the current generated code is global across eligible Catalogs, while historical per-Catalog records remain compatibility-only | digest/pepper; no plaintext persistence; rate limit; current open/unexpired Catalog eligibility | `convex/catalogAccess.ts`, `convex/lib/accessCodes.ts` | security/access tests, `convex/catalog-discovery.test.ts` |
| Catalog access period | deprecated historical, inactive/expired compatibility data | no active Admin create/attach/detach/revoke path; old period sessions remain readable only through their existing compatibility guards | historical period relation is not required for global-code access; period status/link/session guards still protect old sessions | `convex/catalogAccess.ts`, `convex/schema.ts` | `convex/catalog-discovery.test.ts`, legacy access tests |
| Catalog access session | active, expired, revoked | issue on unlock; current global sessions select eligible Catalogs and start on the eligible Catalog recorded by the generated code; legacy Catalog and period sessions remain compatible | session digest/expiry/revocation; global sessions re-check Catalog eligibility; legacy period sessions retain period/link guards | `convex/catalogAccess.ts`, `convex/lib/sessions.ts` | access/session tests, `convex/catalog-discovery.test.ts` |
