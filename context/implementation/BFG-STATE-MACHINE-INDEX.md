...
| Catalog access session | active, expired, revoked | issue on unlock; current global sessions select eligible Catalogs; legacy Catalog and period sessions remain compatible | session digest/expiry/revocation; global sessions re-check Catalog eligibility; legacy period sessions retain period/link guards | `convex/catalogAccess.ts`, `convex/lib/sessions.ts` | access/session tests, `convex/catalog-discovery.test.ts` |
