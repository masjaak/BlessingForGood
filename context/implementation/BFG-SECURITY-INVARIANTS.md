...
| SEC-10 | Current Secret Catalog redemption is global across currently eligible open/unexpired Catalogs, expiring, revocable, and rate-limited; each requested Catalog is rechecked server-side. Historical per-Catalog and period sessions retain their existing guards. | `catalogAccess` sessions/attempt tables and server validators | eligible/ineligible Catalog, expired/revoked/rotated code, brute-force threshold, session replay tests |
