...
| SEC-21 | General Admin System visibility/access uses the canonical active `appUsers.role` / capability model, never a personal email allowlist; read-only audit is Admin-accessible while users/settings and stronger sensitive operations retain their Owner-only guards. | `convex/lib/auth.ts`, `src/components/admin-nav.tsx`, `src/app/admin/audit/page.tsx`, `auditEvents.list` | equivalent active Admins see System/audit; customer, inactive, and non-Admin identities are denied; route and server permission agree |
