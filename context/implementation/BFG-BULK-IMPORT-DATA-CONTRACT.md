...
| Variant | `IN_SCOPE` | One row per canonical Book format: `BB`, `PB`, `HB`, `Cards`, `Pack`, `Slipcase HB`, `Slipcase PB`, `Boxset PB`, `Boxset HB`, or `FLEXIBOUND`. |
...
| `format` | Enum text | Yes | `BB`, `PB`, `HB`, or an additive label | Trim and case-insensitive canonicalization. | Must be one of the current canonical `BB`, `PB`, `HB`, `Cards`, `Pack`, `Slipcase HB`, `Slipcase PB`, `Boxset PB`, `Boxset HB`, or `FLEXIBOUND` values. | `bookVariants.format`. | Book Master + format. | `format must use one of the canonical Book format values` / `variant already exists for this Book Master`. |
