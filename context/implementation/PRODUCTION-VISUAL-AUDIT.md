...
| `/admin/users`                  | PASS              | FAIL                  | Admin 10 language            | Extended          | FAIL    | N/A     | PARTIAL | FAIL       | FAIL      | FAIL    | FAIL       | FAIL        | PARTIAL      | FAIL       | Keep Admin operational status access and Owner-only role/invitation controls in the corrected Admin shell.                |
...
| Secret Catalog codes and grants                               | FUNCTIONAL SECURITY CHANGED | Token-only public gateway now creates scoped opaque sessions; legacy active-member grants and Convex authorization remain intact. |
...
| Secret Catalog token gateway  | PASS                  | Public code-entry screen, scoped token error state, and private projection remains server-gated           |
| Account signed-out states     | PASS                  | Account gate, Buku Saya lock, Tagihan lock, and `Ke Akun` flows rendered at 390px                         |
