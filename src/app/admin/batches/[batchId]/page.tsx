...
function AdminBatchDetail() {
  ...
  const { currentBatch, linkCatalog, unlinkCatalog, archiveBatch, updateShipmentStage } = useOperations();
  const [catalogId, setCatalogId] = useState("");
  const [message, setMessage] = useState("");
  if (dataSource !== "convex")
    return <div className="state-panel">Persistent batches are available in Convex Preview.</div>;
  if (!currentBatch) {
    return (
      <EmptyState
            ...
            {currentBatch.assignments.length ? (
              currentBatch.assignments.map((assignment) => (
                <div className="summary-line" key={assignment.assignmentId}>
                  ...
                  <span className="subtle">{assignment.orderId}</span>
                </div>
              ))
            ) : (
