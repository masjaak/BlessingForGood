const DEFAULT_URL = "https://www.blessingforgood.com";
const DEFAULT_LEVELS = [1, 5, 10, 100, 250, 500, 750, 1000];
const ROUTES = ["/", "/how-to-order", "/join", "/catalog", "/ready-stock"];

function option(name, fallback) {
  const index = process.argv.indexOf(name);
  return index === -1 ? fallback : process.argv[index + 1];
}

function numbers(value) {
  return String(value)
    .split(",")
    .map((item) => Number(item.trim()))
    .filter((item) => Number.isInteger(item) && item > 0);
}

function paths(value) {
  return String(value)
    .split(",")
    .map((item) => item.trim())
    .filter((item) => item.startsWith("/"));
}

function numberOption(name, fallback) {
  const value = Number(option(name, fallback));
  if (!Number.isFinite(value) || value <= 0) throw new Error(`Invalid ${name}`);
  return value;
}

function percentile(values, ratio) {
  if (!values.length) return 0;
  return values[Math.min(values.length - 1, Math.ceil(values.length * ratio) - 1)];
}

function sleep(milliseconds) {
  return new Promise((resolve) => setTimeout(resolve, milliseconds));
}

async function fetchRoute(baseUrl, route, timeoutMs) {
  const startedAt = performance.now();
  try {
    const response = await fetch(new URL(route, baseUrl), {
      redirect: "manual",
      signal: AbortSignal.timeout(timeoutMs),
    });
    await response.arrayBuffer();
    return {
      status: response.status,
      latencyMs: performance.now() - startedAt,
      ok: response.status >= 200 && response.status < 300,
    };
  } catch (error) {
    return {
      status: 0,
      latencyMs: performance.now() - startedAt,
      ok: false,
      timeout: error instanceof Error && /Abort|Timeout/.test(error.name),
      error: error instanceof Error ? error.name : "fetch_error",
    };
  }
}

async function runLevel(baseUrl, profile, routes, users, durationMs, intervalMs, timeoutMs, thresholds) {
  const results = [];
  let nextRoute = 0;
  const startedAt = Date.now();
  const stopAt = startedAt + durationMs;
  await Promise.all(
    Array.from({ length: users }, async (_, userIndex) => {
      while (Date.now() < stopAt) {
        const route = routes[(userIndex + nextRoute++) % routes.length];
        const result = await fetchRoute(baseUrl, route, timeoutMs);
        results.push({ ...result, route });
        const remaining = intervalMs - result.latencyMs;
        if (remaining > 0 && Date.now() < stopAt) await sleep(remaining);
      }
    }),
  );
  const latencies = results.map((result) => result.latencyMs).sort((a, b) => a - b);
  const errors = results.filter((result) => !result.ok);
  const fourXX = results.filter((result) => result.status >= 400 && result.status < 500).length;
  const fiveXX = results.filter((result) => result.status >= 500).length;
  const throttles = results.filter((result) => result.status === 429).length;
  const errorPercent = (errors.length / Math.max(1, results.length)) * 100;
  const stopReasons = [
    ...(fiveXX > 0 ? ["5xx"] : []),
    ...(errorPercent >= thresholds.maxErrorPercent ? ["error_rate"] : []),
    ...(percentile(latencies, 0.95) > thresholds.p95Ms ? ["p95"] : []),
    ...(percentile(latencies, 0.99) > thresholds.p99Ms ? ["p99"] : []),
  ];
  const elapsedMs = Math.max(1, Date.now() - startedAt);
  return {
    users,
    profile,
    durationMs,
    requests: results.length,
    successfulRequests: results.length - errors.length,
    rps: Number((results.length / (elapsedMs / 1000)).toFixed(2)),
    p50Ms: Math.round(percentile(latencies, 0.5)),
    p95Ms: Math.round(percentile(latencies, 0.95)),
    p99Ms: Math.round(percentile(latencies, 0.99)),
    errorPercent: Number(errorPercent.toFixed(2)),
    fourXX,
    throttles429: throttles,
    errors5xx: fiveXX,
    timeouts: results.filter((result) => result.timeout).length,
    connectionFailures: results.filter((result) => result.status === 0).length,
    statuses: Object.fromEntries(
      Object.entries(Object.groupBy(results, (result) => String(result.status))).map(([status, rows]) => [
        status,
        rows.length,
      ]),
    ),
    queue: "not_observable_from_edge_harness",
    stopReasons,
    result: stopReasons.length ? "STOP" : "PASS",
  };
}

const baseUrl = option("--url", DEFAULT_URL);
const profile = option("--profile", "A_READ_HEAVY_PUBLIC_HTTP");
const routes = paths(option("--routes", ROUTES.join(",")));
const levels = numbers(option("--levels", DEFAULT_LEVELS.join(",")));
const durationMs = numberOption("--duration-ms", "3000");
const intervalMs = numberOption("--interval-ms", "1000");
const timeoutMs = numberOption("--timeout-ms", "10000");
const thresholds = {
  p95Ms: numberOption("--p95-ms", "2000"),
  p99Ms: numberOption("--p99-ms", "5000"),
  maxErrorPercent: numberOption("--max-error-percent", "1"),
};

if (profile !== "A_READ_HEAVY_PUBLIC_HTTP") {
  throw new Error(
    "Only the safe public read profile is implemented; authenticated and write profiles need isolated fixtures",
  );
}
if (!routes.length) throw new Error("Invalid --routes");
if (!levels.length) throw new Error("Invalid --levels");

for (const route of routes) await fetchRoute(baseUrl, route, timeoutMs);
console.log(
  JSON.stringify({
    phase: "warmup",
    profile,
    routes,
    durationMs,
    intervalMs,
    timeoutMs,
    thresholds,
  }),
);

for (const users of levels) {
  const result = await runLevel(baseUrl, profile, routes, users, durationMs, intervalMs, timeoutMs, thresholds);
  console.log(JSON.stringify(result));
  if (result.result === "STOP") {
    console.log(JSON.stringify({ stopReasons: result.stopReasons, stoppedAt: users }));
    break;
  }
}
